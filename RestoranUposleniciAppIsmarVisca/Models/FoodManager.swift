...
class FoodManager {
        ...
        FirebaseAuth.Auth.auth().createUser(withEmail: userToCreate.email, password: password, completion: { result, error in
            if error != nil {
                ...
                case .accountExistsWithDifferentCredential, .credentialAlreadyInUse, .emailAlreadyInUse:
                    self.checkIfIsCustomerNotEmployee(userToCreate)
                default:
    ...
    private func checkIfIsCustomerNotEmployee(_ user2: User) {
        self.db.collection("users").whereField("email", isEqualTo: user2.email)
            .getDocuments() { (querySnapshot, err) in
                if err == nil {
                    ...
                    if isCustomer && isEmployee == false {
